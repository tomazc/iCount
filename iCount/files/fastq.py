...
#    Q_PHRED = [
#        10.0 * math.log(math.pow(10.0, x/10.0) + 1.0, 10.0) for x in Q_solexa
#        ]
...
def _determine_format(fname):
    ...
    format = 'L'
    quals = set()
    for rec_cn, per, r in _reader(fname, _sl2l):
...
class Read:
    ...
    def __str__(self):
        return '{0:s}\n{1:s}\n{2:s}\n{3:s}\n'.format(self.r_id, self.r_seq,
                                                     self.r_plus, self.r_qual)
...
class Writer:
    ...
    def write(self, r_id, r_seq, r_plus, r_qual):
        self.f.write(
            '{0:s}\n{1:s}\n{2:s}\n{3:s}\n'.format(r_id, r_seq, r_plus, r_qual)
        )
